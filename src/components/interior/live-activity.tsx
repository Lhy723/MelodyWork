...
export function LiveActivity({
  ...
  useIsoLayoutEffect(() => {
    ...
    const read = () => {
      ...
      if (compact) {
        sizes.current.c = {
          w: compact.offsetWidth,
          h: compact.offsetHeight,
        };
      }
      if (expandedContent) {
        sizes.current.e = {
          w: expandedContent.offsetWidth,
          h: expandedContent.offsetHeight,
        };
      }
      apply(expanded);
    };
...
const LiveActivityContext = createContext<UseLiveActivityReturn | null>(null);

export function LiveActivityProvider({ children }: { children: ReactNode }) {
  const pod = useLiveActivity({ linger: 2400 });

  return (
    <LiveActivityContext.Provider value={pod}>
      <div className="pointer-events-none fixed inset-x-0 top-[calc(var(--harness-window-titlebar-height)+0.75rem)] z-[60]">
        <LiveActivity
          activity={pod.activity}
          dismissLabel="关闭活动"
          label="后台活动"
          onDismiss={pod.dismiss}
          width={320}
        />
      </div>
      {children}
...
export function useGlobalLiveActivity(): UseLiveActivityReturn {
  const pod = useContext(LiveActivityContext);
  if (!pod) {
    throw new Error(
      "useGlobalLiveActivity must be used inside LiveActivityProvider",
    );
  }
  return pod;
}
