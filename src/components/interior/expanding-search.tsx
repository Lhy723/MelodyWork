...
type ExpandingSearchSize = "default" | "sm";
...
export function ExpandingSearch({
  ...
  const expanded = Math.max(sizeConfig.collapsed, track);
  const rightInset =
    sizeConfig.clearSlot +
    (resultCount === undefined ? 0 : sizeConfig.countSlot);
  const inner = Math.max(0, expanded - sizeConfig.inputLeft - rightInset);
  ...
  return (
    <div
      ref={trackRef}
      role="search"
      className={`relative ${sizeConfig.heightClass} w-full ${className}`}
      {...rootProps}
    >
      <motion.div
        initial={false}
        animate={{ width: open ? expanded : sizeConfig.collapsed }}
        ...
        } overflow-hidden border-2 ${sizeConfig.radiusClass} transition-[background-color,border-color,box-shadow] duration-150 ${
          focused
      ...
      >
        <motion.input
          ...
          style={{
            width: inner,
            left: sizeConfig.inputLeft,
            lineHeight: sizeConfig.collapsed + "px",
          }}
          initial={false}
          ...
          className={`absolute inset-y-0 bg-transparent ${sizeConfig.inputClass} text-stone-700 outline-none focus-visible:outline-none placeholder:text-stone-400 dark:text-stone-200 dark:placeholder:text-stone-500 [&::-webkit-search-cancel-button]:appearance-none [&::-webkit-search-decoration]:appearance-none`}
        />
        ...
        >
          {resultCount === undefined ? null : (
            <span
              aria-hidden
              className={`${sizeConfig.countClass} truncate text-right font-mono text-[9.5px] tabular-nums text-stone-500 dark:text-stone-400`}
            >
              {filled ? resultCount : ""}
            </span>
          )}
          ...
          <motion.button
            ...
            style={{
              width: sizeConfig.clearSize,
              height: sizeConfig.clearSize,
            }}
            className={`grid place-items-center rounded-[6px] text-stone-500 outline-none focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-[#4568FF] dark:text-stone-400 dark:focus-visible:outline-[#93B0FF] ${
      ...
      <motion.button
        ...
        initial={false}
        animate={{
          x: align === "right" && open ? -(expanded - sizeConfig.collapsed) : 0,
        }}
        transition={shellMotion}
        className={`absolute inset-y-0 z-10 grid place-items-center ${sizeConfig.triggerClass} text-stone-500 outline-none focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-[#4568FF] disabled:opacity-50 dark:text-stone-400 dark:focus-visible:outline-[#93B0FF] ${
          align === "right" ? "right-0" : "left-0"
