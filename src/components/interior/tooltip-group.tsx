...
function stop(timer: Timer): Timer {
...
function createTooltipStore(getTiming: () => TooltipTiming): TooltipStore {
  ...
  groupCount += 1;
  const seat = `tooltip-seat-${groupCount}`;
...
export function TooltipGroup({
  ...
  const timing = useRef<TooltipTiming>({
    openDelay,
    closeDelay,
    skipDelay,
  });
...
export function useTooltip({
  ...
  const triggerProps: TooltipTriggerProps = {
    ...
    onFocus: (event) => {
      if (disabled || !isKeyboardFocus(event.currentTarget)) return;
...
type TriggerChild = ReactElement<
  HTMLAttributes<HTMLElement> & { "aria-describedby"?: string }
>;
...
export function Tooltip({
  ...
  const { open, seat, skipped, tooltipId, travel, triggerProps } = useTooltip({
    closeDelay,
    disabled,
    openDelay,
    skipDelay,
        ...
        <AnimatePresence>
          {open ? (
            <motion.span
              ...
              className={cn(
                "relative w-max max-w-[220px] shrink-0 overflow-hidden rounded-[8px] px-2 py-1 text-[11.5px] font-medium leading-snug text-stone-700 dark:text-stone-100",
                contentClassName,
              )}
              exit={
                reduced
