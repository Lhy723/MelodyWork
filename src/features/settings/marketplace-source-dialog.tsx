...
export function MarketplaceSourceDialog({
  ...
  return (
    <Drawer
      description="保存后会写入用户级 Melody 配置，并立即同步和扫描插件。"
      ...
      open={open}
      side="right"
      title={originalName ? "编辑 Marketplace" : "添加 Marketplace"}
      width={520}
    >
      {error ? (
        <p
          aria-live="assertive"
          className="mb-4 rounded-lg bg-destructive/5 px-3 py-2 text-destructive text-xs"
          role="alert"
        >
          {error}
        </p>
